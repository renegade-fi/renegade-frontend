...
function hasReceiving(
    ...
    return "receiving" in data && typeof data.receiving === "object" && data.receiving !== null;
}
...
export function useBridgeConfirmation(
    ...
    const { data, status } = useQuery({
        ...
        refetchInterval: (query) => {
            ...
            return status && ["INVALID", "DONE", "FAILED"].includes(status) ? false : 1000;
        },
    });
    ...
    const processedData = React.useMemo(() => {
        ...
        return {
            ...
            lifiExplorerLink: hasLifiExplorerLink(data) ? data.bridgeExplorerLink : "",
    ...
    React.useEffect(() => {
        if (processedData && processedData.status === "DONE" && hash && !isConfirmationHandled) {
