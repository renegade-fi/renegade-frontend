...
export function WETHForm({
    ...
    const ethL2BalanceLabel = formatNumber(ethL2Balance?.value ?? BigInt(0), 18, true);
    ...
    const userHasEthL1Balance = Boolean(ethL1BalanceStatus === "success" && ethL1Balance);

    // Calculate the minimum ETH to keep unwrapped for gas fees
    const minEthToKeepUnwrapped = basePerQuotePrice ?? BigInt(4e15);

    const combinedBalance = (l2Balance ?? BigInt(0)) + (ethL2Balance?.value ?? BigInt(0));
    ...
    const wrapRequired =
        parseEther(amount) > (l2Balance ?? BigInt(0)) && parseEther(amount) <= combinedBalance;
    ...
    const wrapConfirmationStatus = useTransactionConfirmation(wrapHash, async () => {
    ...
    // Approve deposit
    const { data: allowanceRequired, queryKey: wethAllowanceQueryKey } = useAllowanceRequired({
    ...
    const unwrapConfirmationStatus = useTransactionConfirmation(unwrapHash, async () => {
        queryClient.invalidateQueries({ queryKey: l2BalanceQueryKey });
        queryClient.invalidateQueries({ queryKey: ethL2BalanceQueryKey });
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        const isAmountSufficient = checkAmount(queryClient, values.amount, WETH_L2_TOKEN.address);
    ...
    const maxValue = isDeposit ? formattedMaxAmountToWrap : formattedRenegadeBalance;
    ...
    const renderPrompts = () => {
            ...
            return (
                <>
                    ...
                    <ReviewWrap
                        ...
                        wrapAmount={parseEther(form.getValues("amount")) - (l2Balance ?? BigInt(0))}
                    />
                </>
            );
        }
        ...
        if (!isBase && (userHasWethL1Balance || userHasEthL1Balance)) {
            return (
                <BridgePrompt formattedL1Balance={formattedWethL1Balance} token={WETH_L1_TOKEN} />
            );
        }
    };
    ...
    if (steps.length > 0) {
        ...
        } else if (unwrapStatus === "error" || unwrapConfirmationStatus === "error") {
