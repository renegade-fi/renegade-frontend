...
import { ADDITIONAL_TOKENS, ETHEREUM_TOKENS, resolveAddress, SOLANA_TOKENS } from "@/lib/token";
...
export function USDCForm({
    ...
    const [switchChainError, setSwitchChainError] = React.useState<Error | null>(null);
    ...
    const combinedBalance = (usdcL2Balance ?? BigInt(0)) + (usdceL2Balance ?? BigInt(0));
    const formattedCombinedBalance = formatUnits(
        combinedBalance ?? BigInt(0),
        USDC_L2_TOKEN.decimals,
    );
    const maxValue = network === mainnet.id ? formattedUsdcL1Balance : formattedCombinedBalance;

    const { snapshot, captureSnapshot } = useSwapState(form, usdcL2Balance, usdceL2Balance);
    ...
    const switchChainAndInvoke = async (chainId: number | undefined, fn: () => void) => {
        if (chainId) {
    ...
    const bridgeRequired = React.useMemo(() => {
        ...
        return isFirstStep && isCrosschainTransfer && hasValidAmount && hasSolanaWallet;
    ...
    const {
        ...
        fromMint: network === mainnet.id ? USDC_L1_TOKEN.address : USDC_SOLANA_TOKEN,
        toChain: chain?.id,
    ...
    // Approve swap
    const { data: swapAllowanceRequired, queryKey: swapAllowanceQueryKey } = useAllowanceRequired({
        amount: formatUnits(BigInt(swapQuote?.estimate.fromAmount ?? 0), USDCE_L2_TOKEN.decimals),
    ...
    const approveSwapConfirmationStatus = useTransactionConfirmation(approveSwapHash, async () => {
    ...
    const { data: allowanceRequired, queryKey: usdcL2AllowanceQueryKey } = useReadErc20Allowance({
        address: USDC_L2_TOKEN.address,
        args: [address ?? "0x", chainId ? getSDKConfig(chainId).permit2Address : "0x"],
        chainId: chain?.id,
        query: {
            select: (data) => {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        const isAmountSufficient = checkAmount(
            ...
            swapRequired
                ? formatUnits(
                      BigInt(swapQuote?.estimate.toAmountMin ?? 0) + (usdcL2Balance ?? BigInt(0)),
    ...
    const renderTransferOptions = () => {
        if (bridgeRequired) {
            return (
                <>
                    <Separator />
                    <ReviewBridge error={bridgeQuoteError} quote={bridgeQuote} />
                </>
            );
        }
        ...
        if (swapRequired) {
            return (
                <>
                    <Separator />
                    <ReviewSwap error={swapQuoteError} quote={swapQuote} />
                </>
            );
        }
    ...
    const hideMaxButton = !mint || maxValue === "0" || amount.toString() === maxValue;

    const isSubmitDisabled =
        !form.formState.isValid ||
        isMaxBalances ||
        (swapRequired && (isQuoteFetching || !swapQuote)) ||
        (bridgeRequired && (bridgeQuoteFetchStatus === "fetching" || !bridgeQuote)) ||
