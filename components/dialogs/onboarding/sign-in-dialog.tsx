...
import { useCurrentChain, useRememberMe } from "@/providers/state-provider/hooks";
...
export function SignInDialog({
    ...
    const [currentStep, setCurrentStep] = React.useState<number | undefined>(undefined);
    ...
    const buttonText = React.useMemo(() => {
        ...
        if (steps.some((step) => step.status === "error")) {
            ...
            if (isNonDeterministicWalletError(error?.error)) return "Connect a new wallet";
            return "Try again";
        }
            ...
            >
                <DialogContent className="gap-0 p-0 sm:max-w-[425px]">
                    <DialogHeader className="px-6 pt-6">
                        <DialogTitle className="font-extended">Sign in to Renegade</DialogTitle>
                        <DialogDescription>
                            Verify wallet ownership and compatibility before signing in. Signatures
                            are free and do not send a transaction.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-6 p-6">
                        <div className="space-y-3 border p-4">{getSteps(steps, currentStep)}</div>
        ...
        >
            <DialogContent className="h-dvh gap-0">
                <DialogHeader className="text-left">
                    ...
                    <DialogDescription>
                        Verify wallet ownership and compatibility before signing in. Signatures are
                        free and do not send a transaction.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button
...
function RememberMe() {
    ...
    return (
        <div className="flex items-center space-x-2">
            ...
            />
            <Label className="text-muted-foreground" htmlFor="remember-me">
                Remember me
            </Label>
        </div>
    );
...
function getIcon(status: MutationStatus, step: number, currentStep: number | undefined) {
