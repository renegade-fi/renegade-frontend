...
import { MAINTENANCE_MESSAGES, type MaintenanceMessageKey } from "@/lib/constants/maintenance";
...
export function MaintenanceButtonWrapper({
    ...
    const isCriticalMaintenance =
        isProduction && maintenanceMode?.enabled && maintenanceMode.severity === "critical";
    ...
    return (
        <ResponsiveTooltip>
            ...
            <ResponsiveTooltipContent className={isCriticalMaintenance ? "visible" : "invisible"}>
                {maintenanceMessage}
            </ResponsiveTooltipContent>
        </ResponsiveTooltip>
    );
