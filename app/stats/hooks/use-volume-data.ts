...
type UseHistoricalVolumeResult = UseQueryResult<VolumeDataPoint[], Error> & {
  queryKey: readonly [
    "stats",
    "historical-volume",
    { from: number; to: number; interval: number },
  ]
}
...
export function useVolumeData({
  ...
  to,
  interval,
}: VolumeParams): UseHistoricalVolumeResult {
  ...
  const queryKey = [
    "stats",
    "historical-volume",
    { from, to: finalTo, interval: finalInterval },
  ] as const
  ...
  return {
    ...useQuery<VolumeDataPoint[], Error>({
      queryKey,
      queryFn: () =>
        getHistoricalVolume({ from, to: finalTo, interval: finalInterval }),
      staleTime: Infinity,
    }),
    queryKey,
  }
