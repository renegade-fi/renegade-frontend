...
type UseCumulativeVolumeResult = UseQueryResult<VolumeDataPoint[], Error> & {
    queryKey: readonly ["stats", "cumulative-volume", { from: number; to: number }];
};
 ...
 */
export function useCumulativeVolume({ from, to }: VolumeParams): UseCumulativeVolumeResult {
    const now = Math.floor(Date.now() / 1000);
    const finalTo = to ?? now;

    const queryKey = ["stats", "cumulative-volume", { from, to: finalTo }] as const;
