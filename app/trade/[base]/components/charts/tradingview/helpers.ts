...
export async function fetchBarsForPeriod(
    ...
    const timeChunks = splitTimeRange(adjustedStartTimeMs, endTimeMs, MAX_PARALLEL_REQUESTS);
    ...
    const uniqueBars = Array.from(new Map(allBars.map((bar) => [bar.time, bar])).values()).sort(
        (a, b) => a.time - b.time,
    );

    if (uniqueBars.length < countBack) {
        console.warn(`Not enough bars fetched: ${uniqueBars.length}, requested ${countBack}`);
    }
    return uniqueBars;
}
...
async function fetchBarsForTimeChunk(
    ...
    if (startTimeMs <= 0) {
        ...
        const [firstHalf, secondHalf] = await Promise.all([
            ...
            fetchBarsForTimeChunk(ticker, exchange, midTimeMs, endTimeMs, timeInterval, countBack),
        ]);
 ...
 */
function splitTimeRange(startTimeMs: number, endTimeMs: number, maxChunks: number) {
