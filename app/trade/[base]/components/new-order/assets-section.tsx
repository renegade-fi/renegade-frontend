...
export function AssetsSectionWithDepositButton({ base }: { base: `0x${string}` }) {
    ...
    const { data } = useBackOfQueueWallet({
        query: {
            select: (data) =>
                !data.balances.find((balance) => balance.mint === baseToken?.address)?.amount &&
                !data.balances.find((balance) => balance.mint === quoteToken?.address)?.amount,
        },
    });
    return (
        <div className="flex">
...
export function AssetsSection({ base, quote }: { base: `0x${string}`; quote: `0x${string}` }) {
    const baseToken = resolveAddress(base);
    const quoteToken = resolveAddress(quote);

    return (
        <>
            {baseToken ? <Row mint={baseToken.address} /> : <RowFallback ticker={base} />}
            {quoteToken ? <Row mint={quoteToken.address} /> : <RowFallback ticker={quote} />}
        </>
    );
}
    ...
    return (
        <div className="flex justify-between">
            <div className="flex items-center space-x-2">
                <TokenIcon size={16} ticker={ticker} />
...
function Row({ mint, disabled }: { mint: `0x${string}`; disabled?: boolean }) {
    ...
    const formattedBalance = isLoading ? "--" : formatNumber(balance, token.decimals, true);
    ...
    const formattedUsdPriceLabel = isLoading ? "--" : formatCurrencyFromString(formattedUsdPrice);
