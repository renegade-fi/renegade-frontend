...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!res.ok) {
            throw new Error(`Bot server responded with status ${res.status}: ${res.statusText}`);
        }

        const data = (await res.json()) as BotServerResponse;

        return Response.json({ data: { estimatedMs: data.data } } satisfies TimeToFillResponse, {
            status: 200,
        });
    } catch (error) {
        ...
        return Response.json(
            {
                error: error instanceof Error ? error.message : "Failed to fetch time to fill",
            } satisfies TimeToFillResponse,
            { status: 500 },
        );
    }
