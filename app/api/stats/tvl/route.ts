...
export async function GET() {
  try {
    ...
    const darkpoolContract = process.env.NEXT_PUBLIC_DARKPOOL_CONTRACT as `0x${string}`;
    ...
    const tvlData = await Promise.all(tokens.map(async (token) => {
      try {
...
async function fetchTvl(tokenAddress: `0x${string}`, rpcUrl: string, darkpoolContract: `0x${string}`): Promise<bigint> {
