...
import { HISTORICAL_VOLUME_KEY_PREFIX, HISTORICAL_VOLUME_SET_KEY } from "@/app/api/stats/constants"
...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        console.log(`Fetching match volume from ${from} to ${to} with interval ${interval}`)
        ...
        if (res.status === "ok" && res.series && res.series.length > 0 && res.series[0].pointlist) {
            console.log(`Raw data points: ${JSON.stringify(res.series[0].pointlist.slice(0, 5))}...`) // Log first 5 data points
            ...
            const setPromises = volumeData.map(async (data: VolumeData, index) => {
                ...
                try {
                    ...
                    successCount++
                    if (index % 10 === 0) console.log(`Processed ${index + 1}/${volumeData.length} data points`)
                } catch (error) {
            ...
            console.log(`Volume data successfully written to KV store. ${successCount}/${volumeData.length} points processed successfully`)
            ...
            return new Response(JSON.stringify({ error: res.error || "No data found" }), { status: 500 })
        }
    } catch (error) {
