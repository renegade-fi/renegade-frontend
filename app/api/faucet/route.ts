...
const abi = parseAbi(["function mint(address _address, uint256 value) external"]);
...
export async function POST(request: Request) {
    ...
    try {
        ...
        for (const { ticker, amount } of TOKENS_TO_FUND) {
            ...
            await mintUpTo(recipient, token.address, parseUnits(amount, token.decimals));
        }
...
async function fundEth(recipient: `0x${string}`, amount: bigint): Promise<void> {
...
async function mintUpTo(recipientAddr: `0x${string}`, token: `0x${string}`, amount: bigint) {
    ...
    if (mintAmount > 0) {
        ...
        console.log(
            `No minting needed for ${resolveAddress(token).ticker} for address ${recipientAddr}`,
        );
    }
}
