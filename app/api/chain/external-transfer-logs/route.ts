import { BucketData, ExternalTransferData, INFLOWS_KEY, INFLOWS_SET_KEY } from "@/app/api/chain/constants";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const dataPromises = transactionHashes.map(hash => kv.get(`${INFLOWS_KEY}:${hash}`))
    const data = await Promise.all(dataPromises)

    // Filter out any null values and ensure the type is ExternalTransferData
    const validData: ExternalTransferData[] = data.filter((item): item is ExternalTransferData =>
    ...
    const buckets = validData.reduce((acc, item) => {
    ...
    const sortedBucketData = Object.values(buckets).sort((a, b) =>
