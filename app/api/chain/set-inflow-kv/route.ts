...
import { createPublicClient, http, parseAbiItem, isAddress, formatUnits } from "viem"
...
async function getBlockTimestamps(blockNumbers: bigint[]): Promise<Map<bigint, bigint>> {
    ...
    for (let i = 0; i < blockNumbers.length; i += BLOCK_CHUNK_SIZE) {
        ...
        const blockPromises = chunk.map(blockNumber => viemClient.getBlock({ blockNumber }));
        const blocks = await Promise.all(blockPromises);
        blocks.forEach(block => blockNumberToTimestamp.set(block.number, block.timestamp));
    }
...
export async function GET(req: NextRequest) {
    try {
        ...
        let fromBlock = BigInt(await kv.get(LAST_PROCESSED_BLOCK_KEY) || process.env.FROM_BLOCK || 0)
        ...
        const uniqueBlockNumbers = Array.from(new Set(logs.map(log => log.blockNumber)))
            ...
            if (mint && isAddress(mint)) {
                ...
                if (price && token && log.args.amount) {
                    ...
                    if (timestamp) {
                        ...
                        await Promise.all([
                            kv.set(`${INFLOWS_KEY}:${log.transactionHash}`, JSON.stringify(data)),
        ...
        const processedResults = results.filter((result): result is ExternalTransferData => result !== null);
